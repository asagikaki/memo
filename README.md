...
BulletManager(Bullet|LaserA|LaserB|LaserC|Enemy 及び派生) 敵・弾の一括処理用
...
配列操作式
○[i:j:k] （内包表記）for(int I = i;I<k;I+=k) で得られるIから成る配列と同じ
○[i:j;k] （内包表記）iからjまでをk分割した配列を返す
